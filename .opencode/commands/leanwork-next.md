...
1. **Procure os artefatos do pipeline** no diretório atual. Locais convencionais (mas verifique também pasta raiz e `docs/` se não achar):
   ...
   - Contexto do agente: `AGENTS.md` na raiz e nas pastas de módulo
...
2. **Para cada artefato encontrado, identifique o estado:**
   ...
   - **Contexto do agente**: existe `AGENTS.md` na raiz? Tem as seções essenciais (Stack, Comandos, Convenções)? Há `<!-- TODO -->` pendentes? Módulos sem `AGENTS.md`?
...
3. **Sintetize o estado** em uma tabela curta para o usuário ver:
   ...
   | AGENTS.md (raiz)         | Comandos com TODO | Rodar `/leanwork-context raiz` |
   ```
...
4. **Sugira UMA próxima ação concreta**, em ordem de prioridade. Não listar opções:
   ...
   **Sugestão paralela — contexto do agente (nunca prioridade principal):**
   ...
   - Projeto tem artefatos do pipeline mas nenhum `AGENTS.md` na raiz: "(Aproveitando: não há `AGENTS.md` na raiz — `/leanwork-context raiz` deixaria os próximos reviews mais completos.)"
   - `AGENTS.md` existe mas tem `<!-- TODO -->` pendentes: "(A seção Comandos do `AGENTS.md` está com TODO — agora que há scaffolding, `/leanwork-context raiz` consegue preencher.)"
   - Módulo novo detectado sem `AGENTS.md`: "(Módulo `X` apareceu desde a última vez — `/leanwork-context modulo X` se achar que vale.)"
   ...
   - Projeto tem código mas nenhuma seção `permission` no `opencode.json`: "(O repositório não tem permissões configuradas — `/leanwork-context permissoes` gera a seção `permission` do `opencode.json` calibrada pela stack.)"
...
- Não gerar nem editar `AGENTS.md` por conta própria. O arquivo é território do dev: apenas sugerir `/leanwork-context` e seguir em frente se ele ignorar.
