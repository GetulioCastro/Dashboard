---
description: Gera a matriz de rastreabilidade ADR ↔ RN ↔ CA ↔ UI ↔ T ↔ R a partir dos artefatos do pipeline SDD. Argumento opcional: arquivo do PRD (se omitido, tenta descobrir).
---
...
- **Status de cada tarefa**: campo `**Status:**` de dentro do bloco `#### T-XX`, com um de quatro valores literais — `Pendente` / `Em andamento` / `Concluído` / `Bloqueado` (ver `.opencode/templates/id-conventions.md`). Três cuidados de leitura:
