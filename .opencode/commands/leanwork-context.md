---
description: Gera ou atualiza o contexto do projeto para agentes — AGENTS.md (raiz e módulos) e regras de permissão no opencode.json. Argumento opcional: raiz | modulo <nome> | permissoes | auditar.
---

# Contexto do projeto para agentes de IA

Gerar ou atualizar o contexto de projeto para agentes de IA: os arquivos `AGENTS.md` (Resumo, Stack, Comandos, Convenções, Restrições, índice da documentação) e as regras de permissão na seção `permission` do `opencode.json`.
...
- `raiz` → gerar/atualizar apenas o `AGENTS.md` da raiz
- `modulo <nome>` → gerar/atualizar apenas o módulo indicado
- `permissoes` → gerar/atualizar apenas a seção `permission` do `opencode.json`
...
**Permissões seguem menor privilégio.** Padrão específico, nunca curinga amplo — `"dotnet build*": "allow"` e jamais `"*": "allow"`. O deny universal de segredos entra sempre. E nunca colocar `docs/` em `deny`: o agente lê PRDs, planos e ADRs o tempo todo.
...
- Arquivos criados, atualizados e inalterados (`AGENTS.md` e/ou `opencode.json`)
...
- Módulos detectados que não receberam `AGENTS.md`, com o motivo
...
- Quando o `reviewer-leanwork` rodou em modo degradado por falta de `AGENTS.md`
