---
description: Executa uma tarefa do plano SDD carregando o contexto declarado (RN/CA/ADR/UI), implementando só o escopo da tarefa e atualizando o Status ao final. Argumento opcional: T-XX (se omitido, pega a próxima tarefa elegível).
---
...
O vocabulário de status é fixo (`Pendente` / `Em andamento` / `Concluído` / `Bloqueado`) e está em `.opencode/templates/id-conventions.md`. Ler o campo `**Status:**` de dentro do bloco `#### T-XX` — o `Status:` do cabeçalho do plano é status de documento, não de tarefa.
...
Ler também o `AGENTS.md` da raiz e o do módulo afetado, se existirem — é onde estão as convenções que o review vai cobrar.
...
Escrever os testes de `Testes a escrever:` **antes** do código de produção da costura correspondente, quando praticável — ver o teste falhar primeiro é o que confirma que ele testa a coisa certa, não um teste que passaria mesmo vazio. Nomear cada um conforme `.opencode/templates/id-conventions.md` — `CA_XX_descricao` para os que provam um cenário. É esse nome que fecha o elo `CA-XX → teste` para o `/leanwork-trace`.
