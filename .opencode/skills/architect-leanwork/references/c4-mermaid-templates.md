...
Este exemplo usa nomes de padrão (não de biblioteca) — o objetivo é ilustrar agrupamento lógico, não prescrever uma arquitetura em camadas específica. Para uma versão com bibliotecas reais de uma stack .NET (MediatR, FluentValidation, EF Core), ver `.opencode/stacks/dotnet/c4-component-example.md`.
