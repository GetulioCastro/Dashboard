...
- **Observabilidade**: há SRE/operação dedicada? Que tipo de visibilidade é esperado?
...
Use Mermaid para todos os diagramas. Exemplos de templates em `.opencode/skills/architect-leanwork/references/c4-mermaid-templates.md`. Prefira `flowchart` para containers e componentes; `sequenceDiagram` para fluxos críticos quando ajudam a explicar uma decisão.
...
- `.opencode/skills/architect-leanwork/references/c4-mermaid-templates.md` — templates Mermaid para os 4 níveis do C4 + sequence + state diagrams, com anti-padrões comuns
- `.opencode/skills/architect-leanwork/references/quality-attributes.md` — catálogo de atributos de qualidade (performance, escalabilidade, disponibilidade, resiliência, segurança, manutenibilidade, observabilidade) com perguntas-gatilho, sinais de prioridade, padrões que atendem, e trade-offs a explicitar
- `.opencode/skills/architect-leanwork/references/architectural-styles.md` — catálogo de estilos arquiteturais (monolito, modular monolith, microsserviços, pipes & filters, EDA, CQRS, event sourcing, microkernel, serverless, REST+gateway) com forças, fraquezas, isomorfismo natural e quando evitar
- `.opencode/skills/architect-leanwork/references/adr-template.md` — template standalone de ADR (quando o usuário preferir ADRs como arquivos separados em vez de inline na proposta)
- `.opencode/stacks/dotnet/pipeline-example.md` — exemplo end-to-end (calibrado em .NET) da mesma demanda nas cinco fases; consultar para ver como um `ADR-XX` desta proposta é citado depois no PRD, no plano e no review
- `.opencode/templates/id-conventions.md` — convenções de IDs do pipeline SDD (ADR-XX, RN-XX, CA-XX, UI-XX, T-XX, R-XX), úteis para padronizar numeração na seção 5 desta proposta
...
> A proposta define stack, restrições e convenções que o agente de IA precisa saber em toda sessão — mas ele não vai ler 300 linhas a cada vez. Se quiser, a skill `context-leanwork` gera um `AGENTS.md` na raiz com o essencial (Resumo, Stack, Convenções, Restrições e índice dos docs), referenciando esta proposta para o detalhe. Execute `/leanwork-context raiz` quando achar melhor.

Mencionar que a seção **Comandos** do `AGENTS.md` só fica completa depois do primeiro scaffolding — antes disso não há comando de build ou teste para detectar.

A sugestão é um convite, não etapa obrigatória. Se o usuário ignorar, seguir normalmente. **Nunca gerar o `AGENTS.md` por conta própria** — o arquivo é território do dev.

---

## Referências

- `.opencode/skills/architect-leanwork/references/c4-mermaid-templates.md` — Templates Mermaid para C4 (Context, Container, Component, Sequence)
- `.opencode/skills/architect-leanwork/references/quality-attributes.md` — Catálogo de atributos de qualidade com perguntas-gatilho e padrões que os atendem
- `.opencode/skills/architect-leanwork/references/architectural-styles.md` — Resumo dos estilos arquiteturais com forças, fraquezas e quando usar
- `.opencode/skills/architect-leanwork/references/adr-template.md` — Template completo de ADR (caso o usuário queira ADRs separados, fora do documento principal)
