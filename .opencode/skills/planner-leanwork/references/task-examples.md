...
Os exemplos abaixo usam pseudocódigo e nomes de camada genéricos — nenhum framework ou biblioteca específica. Para ver os mesmos seis exemplos com código real (.NET, EF Core, MediatR, xUnit, Serilog, LaunchDarkly), ver `.opencode/stacks/dotnet/task-examples.md`.

---

## Princípio de granularidade

Boa tarefa cabe em **1 commit ou 1 PR pequeno**, executável em **30 minutos a 4 horas** — teto canônico declarado em `.opencode/templates/id-conventions.md` (regra de `T-XX`). Acima disso, quebrar. Abaixo disso (10 min), provavelmente é um detalhe que deveria estar embutido em outra tarefa.
...
- Padrão de nomenclatura do projeto usa algum sufixo específico para entidades? Verificar antes em `AGENTS.md`.
```
...
`AGENTS.md` do projeto como ativar/desativar.
