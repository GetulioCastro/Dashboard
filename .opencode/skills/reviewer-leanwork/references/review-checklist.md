...
Calibrado pela stack descoberta na Fase 2 + padrões do `AGENTS.md` do projeto. Princípios universais sempre aplicam; padrões específicos só quando declarados.
...
Lidos do `AGENTS.md` do projeto. Exemplos do que pode estar declarado e virar critério:
...
- **Não impor preferência pessoal** que não está no `AGENTS.md`. Se o projeto não declara que prefere `for` a `forEach`, não criticar.
...
- **Padrão emergente**: o time está estabelecendo um padrão que ainda não está no `AGENTS.md`. Sugerir documentar.
...
| Padrão do AGENTS.md violado sistematicamente | | ✓ | |
