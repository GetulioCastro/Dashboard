...
Procurar `AGENTS.md` na raiz do repositório. Times maduros declaram stack e convenções aqui.
...
Quando o `AGENTS.md` declara padrões, esses padrões viram critério de review automaticamente. Não inventar padrões diferentes mesmo que pareçam melhores — o review é sobre adequação ao projeto, não preferência.

### 3. Inspeção do repositório

Quando não há proposta nem `AGENTS.md`, inspecionar arquivos-pista:
...
> "Não encontrei a proposta arquitetural nem AGENTS.md neste projeto, e a inspeção do repo foi inconclusiva. Qual a stack principal? (linguagem + framework principal + banco)"

Depois de receber a resposta, **sugerir documentar no `AGENTS.md` do projeto** para evitar a pergunta em reviews futuras.
...
**Lidos do `AGENTS.md` do projeto.** A skill não traz padrões pré-definidos por stack. Por quê:
...
### Heurísticas mínimas por stack (quando o `AGENTS.md` não declara)

Se o `AGENTS.md` não existe ou não declara padrões específicos, e a stack foi inferida por inspeção, aplicar apenas heurísticas universais. **Não inventar** que "em .NET deveria ter MediatR" — isso é opinião, não regra do projeto.

O que registrar no relatório nesse caso:

> ⚠️ **Stack detectada por inspeção do repositório** (`.csproj` indica .NET 8). Padrões específicos do projeto não estão documentados em `AGENTS.md` — foram aplicados apenas critérios universais de qualidade. Recomenda-se documentar convenções do time em `AGENTS.md` para futuros reviews.
...
Não fingir conhecimento profundo de stack que não foi declarada no `AGENTS.md`.
...
- Fonte: proposta arquitetural (seção 6.2) + AGENTS.md (raiz)

**Padrões específicos aplicados:**
- MediatR para comandos e queries (declarado em AGENTS.md)
- FluentValidation para validação (declarado em AGENTS.md)
- BusinessException para erros de negócio (declarado em AGENTS.md)
- Naming `XxxCommand`/`XxxHandler` (declarado em AGENTS.md)
```
