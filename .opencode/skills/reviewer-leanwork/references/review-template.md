...
> **Reviewer:** o agente (skill `reviewer-leanwork` v1.0)
> **Data:** [AAAA-MM-DD]
> **Round:** [1 / 2 / 3]
...
- [Banco: ...]
- [Frontend: ...]
- **Fonte:** [proposta arquitetural / AGENTS.md / inspeção de repositório / informação do usuário]

### Padrões específicos aplicados (lidos do projeto)

- [padrão 1, ex.: "MediatR para comandos" (AGENTS.md)]
- [padrão 2, ex.: "BusinessException para erros de negócio" (AGENTS.md)]
- [padrão 3, ex.: "Naming `XxxCommand`/`XxxHandler`" (AGENTS.md)]

*Se o projeto não tem padrões declarados em AGENTS.md, indicar:*

> ⚠️ Padrões específicos da stack não estão documentados em `AGENTS.md`. Foram aplicados apenas critérios universais de qualidade. Recomenda-se documentar convenções do time para futuros reviews.
...
- **Padrão emergente:** [se aplicável — o time estabeleceu um padrão que ainda não está no `AGENTS.md`. Descrever o padrão observado e sugerir `/leanwork-context raiz` para documentá-lo]
- **Contexto do agente desatualizado:** [se aplicável — ex.: `AGENTS.md` declara stack ou convenção que o código já não segue. Sugerir `/leanwork-context auditar`]
...
- [Item 3, ex.: "Atualizar AGENTS.md com padrão emergente identificado em Notas ao processo"]
````
