...
- **Sem invenção.** Se faltar contexto (PRD, plano, AGENTS.md), declara a lacuna no relatório e segue com review degradado, em vez de inventar critérios.
...
2. **Localizar artefatos** do pipeline no projeto (plano, PRD, arquitetura, AGENTS.md)
...
Buscar nas convenções padrão de pasta (ver `.opencode/templates/folder-conventions.md`):
...
- **AGENTS.md do projeto** — raiz do repositório.
...
A skill é stack-agnóstica. A stack vem do projeto, não da skill. Aplicar a cascata descrita em `.opencode/skills/reviewer-leanwork/references/stack-detection.md`:

1. **Proposta arquitetural** (preferência) — seção 6 (Visão arquitetural / Containers) lista tecnologias
2. **AGENTS.md do projeto** — frequentemente declara stack e padrões
...
**Padrões específicos do projeto** vêm do `AGENTS.md` (ou da proposta arquitetural). A skill não traz padrões pré-definidos — lê do projeto e aplica.

Exemplo: se o `AGENTS.md` diz "usamos MediatR para todos os handlers, naming `XxxCommand` / `XxxHandler`, errors via `BusinessException`", então o reviewer aplica isso como critério. Se outro projeto disser "handlers diretos no controller, sem MediatR, errors via Result pattern", aplica esse outro.
...
Detalhes de critérios e perguntas-guia por eixo em `.opencode/skills/reviewer-leanwork/references/review-checklist.md`. Resumo:
...
- Nome de teste segue convenção `CA_XX_descricao` (ver `.opencode/templates/id-conventions.md`)?
...
Aqui entra o conhecimento da stack descoberta na Fase 2 e dos padrões lidos do `AGENTS.md` do projeto.
...
**Critérios específicos da stack:** lidos do `AGENTS.md` do projeto. Se o projeto não tem `AGENTS.md` com padrões definidos, aplicar apenas os universais e registrar no relatório que padrões específicos da stack não foram avaliados por falta de declaração.
...
Use o template em `.opencode/skills/reviewer-leanwork/references/review-template.md`. O relatório:

- Cabeçalho identificando T-XX, PRD/Plano de referência, stack detectada, autor da review (o agente), data
...
`R-XX` é o único ID do pipeline cujo namespace é o arquivo, e não o projeto — os demais valem sob a regra de não reúso, `R-XX` é a exceção a ela (ver `.opencode/templates/id-conventions.md`):
...
- `.opencode/skills/reviewer-leanwork/references/review-template.md` — template completo do relatório de review
- `.opencode/skills/reviewer-leanwork/references/review-checklist.md` — perguntas-guia detalhadas por eixo
- `.opencode/skills/reviewer-leanwork/references/stack-detection.md` — cascata de descoberta da stack com exemplos
- `.opencode/stacks/dotnet/pipeline-example.md` — exemplo end-to-end (calibrado em .NET) da mesma demanda nas cinco fases; consultar para ver como um `R-XX` entra na matriz de rastreabilidade e devolve a tarefa para `Bloqueado`
