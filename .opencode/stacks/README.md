...
O núcleo do plugin (skills, templates, comandos) é stack-agnóstico: pseudocódigo, nomes de padrão genéricos (handler, repositório, validador) e placeholders entre colchetes. Nenhuma skill exige uma stack específica para funcionar — a resolução de stack acontece via proposta arquitetural, `AGENTS.md` do projeto ou inspeção do repositório (ver `references/stack-detection.md` de cada skill).
...
Um arquivo agnóstico que tem versão calibrada inclui um ponteiro explícito para ela (`.opencode/stacks/<nome>/...`). O inverso também vale: todo arquivo em `stacks/` referencia de volta o material agnóstico que ele exemplifica.
